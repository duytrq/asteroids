...
void DrawText(std::string string, std::string Font, int size, int x, int y,Uint8 fR, Uint8 fG, Uint8 fB,Uint8 bR, Uint8 bG, Uint8 bB, bool transparent, double angle)
{
    ...
    if (transparent == true)
        textSurface = TTF_RenderText_Blended(font, string.c_str(), foregroundColor);
    else
        textSurface = TTF_RenderText_Shaded(font, string.c_str(), foregroundColor,backgroundColor);

    SDL_Texture* texture1 = SDL_CreateTextureFromSurface(gRen, textSurface);
    SDL_Rect textLocation = { x, y, textSurface->w, textSurface->h };
    SDL_RenderCopyEx(gRen, texture1, NULL, &textLocation, angle, NULL, SDL_FLIP_NONE);
